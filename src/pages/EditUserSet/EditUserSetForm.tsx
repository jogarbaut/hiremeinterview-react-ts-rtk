...
import { UserQuestion } from '@/features/userQuestionSets/userQuestionSetsTypes';

type Props = {};

const EditUserSetForm = (props: Props) => {
    const [title, setTitle] = useState<string>('');
    const [isFavorite, setIsFavorite] = useState<boolean>(false);
    const [displayError, setDisplayError] = useState<boolean>();
    const [inputArray, setInputArray] = useState<UserQuestion[]>([]);
    ...
    const questionSet = questionsSets.find((questionsSet) => questionsSet.id === id);
    ...
    const handleDelete = () => {
        dispatch(deleteUserQuestionSet(questionSetId));
        navigate('/');
    };
    ...
    return (
        <>
            <div className="my-6 flex w-full items-center justify-start gap-8">
                ...
                {displayError &&
                    (!title || !inputArray.every((input) => input.question.length !== 0)) && (
                        <div className="rounded-full border-2 border-transparent bg-red-200 px-4 py-2 text-sm font-bold">
                            Oh no! There are empty items in your user set
                        </div>
                    )}
            </div>
            <form>
                ...
                <div
                    className={`${
                        title.length === 0 && displayError ? 'border-red-300' : 'border-transparent'
...
export default EditUserSetForm;
